// ============================================
// PATCH PARA INTEGRAÇÃO NIVUSPAY
// Adicione este código no início do seu arquivo JavaScript
// ============================================
...
// ============================================
// SUBSTITUIR A FUNÇÃO ORIGINAL DE CRIAR PAGAMENTO
// Localize onde está: _.Z.create() e substitua por createNivusPayment()
// ============================================

async function createNivusPayment(paymentData, urlParams) {
  try {
    console.log('🚀 Criando pagamento NivusPay', paymentData);

    // Preparar payload para NivusPay
    const nivusPayload = {
      paymentMethod: paymentData.paymentMethod.toLowerCase(), // 'pix', 'boleto', 'credit_card'
      amount: Math.round(paymentData.quantity * paymentData.salesPlanId?.salesPrice || 0),
      ...
      // Items do pedido
      items: [{
        title: paymentData.salesPlanId?.name || 'Produto',
        unitPrice: Math.round(paymentData.salesPlanId?.salesPrice || 0),
      ...
      customer: {
        ...
        phone: paymentData.payerPhone?.replace(/\D/g, ''),
        document: {
          type: 'cpf',
          number: (paymentData.payerCpf || paymentData.payerDocument)?.replace(/\D/g, '')
        }
      }
    };

    // Adicionar items adicionais (orderBumps)
    if (paymentData.additionalSalesPlans?.length > 0) {
      paymentData.additionalSalesPlans.forEach(plan => {
        nivusPayload.items.push({
          title: plan.name,
          unitPrice: Math.round(plan.salesPrice),
          quantity: 1,
          tangible: true
        });
      });
    ...
    // Adaptar resposta para o formato esperado pelo código original
...
// ============================================
// FUNÇÃO AUXILIAR: GERAR QR CODE BASE64
// ============================================
...
async function generateQRCodeBase64(qrCodeText) {
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onloadend = () => resolve(reader.result.split(',')[1]); // Retorna apenas o base64
...
// ============================================
// FUNÇÃO PARA VERIFICAR STATUS (POLLING)
// Substitui a verificação por EventSource
// ============================================
...
// Procure por: await u.Z.create(...)
// E substitua a chamada por:
// ============================================

/**
 * EXEMPLO DE COMO APLICAR NO CÓDIGO ORIGINAL:
 * 
 * ANTES:
 * const { data: K } = await _.Z.create({
 *   payerCpf: ...,
 *   payerName: ...,
 *   ...
 * });
 * 
 * DEPOIS:
 * const { data: K } = await createNivusPayment({
 *   payerCpf: ...,
 *   payerName: ...,
 *   ...
 * });
...
// ============================================
// OVERRIDE DO POLLING DE STATUS
// Substitua a verificação com EventSource por:
// ============================================

function startNivusPaymentPolling(orderId, onSuccess) {
  let checkCount = 0;
  const maxChecks = 360; // 30 minutos (5 segundos * 360)

  const interval = setInterval(async () => {
    const status = await checkNivusPaymentStatus(orderId);
...
// Cole este código logo após as importações
// ============================================

/**
 * INSTRUÇÕES DE INTEGRAÇÃO:
 * 
 * 1. Adicione este arquivo no seu projeto
 * 
 * 2. No arquivo principal do checkout, localize a função que cria o pagamento.
 *    Geralmente está assim:
 *    
 *    const { data: resultado } = await apiOriginal.create({...});
 * 
 * 3. Substitua por:
 *    
 *    const { data: resultado } = await createNivusPayment({...});
 * 
 * 4. Localize onde está o EventSource para verificação de status:
 *    
 *    const eventSource = new EventSource(`${apiUrl}/checkout/events/${orderId}`);
 * 
 * 5. Substitua por:
 *    
 *    const pollingInterval = startNivusPaymentPolling(orderId, (status) => {
 *      // Callback de sucesso
 *      handlePaymentSuccess(status);
 *    });
 * 
 * 6. Teste o fluxo completo de pagamento PIX
 */
